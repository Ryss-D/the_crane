...
- [ ] **WEB-2 — Request flow** *(deps: WEB-1, CUS parity)*
  ...
  *AC: full request lands in the same dispatch pipeline as the app.*
  Built: vehicle-type selector, quote, confirm and redirect-to-tracking all wired through the `CraneApi` seam and covered end to end (mock path) by `RequestPage.test.tsx`. Not done: pickup/dropoff are still plain text inputs behind a `TODO(FND-6)` placeholder box -- no browser geolocation, no Places autocomplete; and the real (`VITE_USE_MOCKS=false`) path is actually broken against the backend today -- `backend/app/schemas/job.py`'s `QuoteRequest`/`JobCreate` require `pickup`/`dropoff` as `{lat, lng}`, but `HttpApi` sends `pickup_address`/`dropoff_address` strings instead, so a real quote/create call would 422. Never verified against a live backend.
